...
main = do
    ...
    listen sock 2
    -- fork off thread for reading from all channel duplicates
    _ <- forkIO $ fix $ \loop -> do
        Msg (_,msg) <- readChan chan
        putStrLn msg
        loop
    -- loop, waiting for connections, and adding them as they come
...
mainLoop sock chan nr = do
    ...
    conn <- accept sock
    -- fork off a thread for the new connetion
    _ <- forkIO (runConn conn chan nr)
    -- wait for another connection, with an incremented id
    mainLoop sock chan $! nr+1

runConn :: (Socket, SockAddr) -> Chan (Msg String) -> Int -> IO ()
runConn (sock,_) chan nr = do
    let broadcast msg = writeChan chan (Msg (nr,msg))
    --Turn the socket into a handle for this for ease of use
    hdl <- socketToHandle sock ReadWriteMode
    hSetBuffering hdl NoBuffering   -- might not be necessary, by default the handle is unbuffered
    ...
    reader <- forkIO $ fix $ \loop -> do
        ...
        loop
    -- loop in current thread to handle input
